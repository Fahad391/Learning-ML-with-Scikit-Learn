...
# Step-3 Scale & Predict using transform and predict only
F_Scaled = scaler.transform(Feature)
DF['Assigned_Cluster'] = Kmeans.predict(F_Scaled)

# Step-4 Extract original centroids from the loaded model
scaled_centers = Kmeans.cluster_centers_
original_centers = scaler.inverse_transform(scaled_centers) # inverse_transform decodes to human readable data
...
for i, row in DF.iterrows():
    ...
    z_spend, z_freq = F_Scaled[i][0], F_Scaled[i][1]
